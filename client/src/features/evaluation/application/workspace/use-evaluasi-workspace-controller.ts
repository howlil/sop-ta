...
import { usePengajuanEvaluasiAktif } from '../../hooks/evaluasi-derived-hooks'
import { useEvaluasiDraft } from '../../hooks/use-evaluasi-draft'
import { useEvaluasiSubmit } from '../../hooks/use-evaluasi-submit'
import {
...
export type EvaluasiWorkspaceFormTab = 'sop' | 'aktivitas' | 'opd'

export type EvaluasiWorkspaceControllerInput =
  | {
      mode: 'opd'
      opdId: string
...
/**
 * Application controller untuk workspace evaluasi.
 * Memiliki selection, form/dialog state, workflow commands, dan projection orchestration.
 * Rendering tetap dimiliki page; DTO/domain mapping tetap pure di model layer.
 */
export function useEvaluasiWorkspaceController(props: EvaluasiWorkspaceControllerInput) {
  ...
  const [activeFormTab, setActiveFormTab] = useState<EvaluasiWorkspaceFormTab>('sop')
